    ...
    for label, valid in zip(labels, labels_valid):
        ...
        if valid:  # exclude all labels from statistics which are not considered valid
            ...
            for detection in detections:
                ...
                label_pol = Polygon(label_box)
                det_pol = Polygon(det_box)
                intersection = label_pol.intersection(det_pol).area
                union = label_pol.union(det_pol).area
...
def compute_performance_stats(det_performance_all):
    ...
    positives, true_positives, false_negatives, false_positives = 0, 0, 0, 0
    
    for det_performance in det_performance_all:
        positives += det_performance[2][0]
